...
public class DatabaseConnection {
	...
	private String userName = "ae8081", password = "rotmos", infomember ="", topMember = "";
	...
	private String kp="", kpTop = "", kPerson ="", knamn= "", infoOfEach="";
...
//		infoAboutEach(medlemmar);
	...
	public void infomember(String query){
			infomember= "";
			topMember="";
			try {
				Connection conn = DriverManager.getConnection("jdbc:mysql://195.178.235.60:3306/ae8081",
				...
				Statement stmt = conn.createStatement();
				ResultSet rs = stmt.executeQuery(query);
				topMember =  " Information om Bandmedlemmarna:\n";
				while(rs.next())
				infomember += rs.getString(1) + ":\t";
	...
	public void infoAboutEach(String query){
		infoOfEach="";
		String ask = "select personliginfo from Medlemmar where Namn =" + "'" + query + "'";
		try {
			Connection conn = DriverManager.getConnection("jdbc:mysql://195.178.235.60:3306/ae8081",
												userName, password);
			Statement stmt = conn.createStatement();
	...
	public String getInfoMember(){
		return topMember + infomember;
	}
	
	public String getEachInfo(){
		return infoOfEach;
	}
